  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      <div className="px-6 md:px-16 py-8">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 mb-8">
          <div className="flex items-center gap-2 flex-wrap">
            {categories.map((cat) => (
              <button
                ...
                className={`cursor-pointer px-4 py-2 rounded-xl text-xs font-semibold transition-all duration-200 ${
                  activeCategory === cat
          ...
          <div className="flex items-center gap-3">
            ...
            <select
              ...
              className="cursor-pointer text-xs font-semibold text-gray-700 border border-gray-200 rounded-xl px-3 py-2 outline-none bg-white hover:border-[#3525d7] transition"
            >
        ...
        {error && (
          <div className="mb-5 rounded-xl border border-yellow-200 bg-yellow-50 px-4 py-3 text-xs font-semibold text-yellow-700">
            {error}
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
            {visibleCourses.map((course) => (
              <div key={course._id || course.id} className="bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-lg transition-shadow duration-300 group">
                <div className="relative h-44 overflow-hidden">
                  <img src={getThumbnail(course)} alt={course.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between mt-10 gap-4">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="cursor-pointer w-8 h-8 rounded-lg border border-gray-200 flex items-center justify-center text-gray-500 hover:border-[#3525d7] hover:text-[#3525d7] transition text-xs"
            >
              Prev
            </button>
